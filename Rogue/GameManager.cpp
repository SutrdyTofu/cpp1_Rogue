...
int GameManager::StageChoice()
{
	...
	for (int i = 0; i < MAP_COL; i++) {
		if(node[i] == 1)
			nodeString += to_string(i + 1) + "." + mapNameList[mapDetail[currentNode/10][i]]+"  ";
	}
...
void GameManager::optimizationMap(State* player) {
	...
	string StringSelection = "1. HP 20% ȸ    ";
	string StringResult = "";
	int playerChoice = -1;
	...
	case 2:
		...
		StringResult = this->skillManager.skillNameList[randSkill_1] + "   ų   ȹ   Ͽ    ϴ .";
		break;
	case 3:
		player->addSkill(randSkill_2);
		StringResult = this->skillManager.skillNameList[randSkill_2] + "   ų   ȹ   Ͽ    ϴ .";
		break;
	default:
		break;
	}
...
void GameManager::LegacyCodeMap(State* player, State enemyList[ENEMY_COUNT]) {
	...
	case 1:
		...
		drawUI.DrawTextUI("   ͵    ü            " + to_string(rand) + "%        ߽  ϴ ", "");
		drawUI.DrawInputUI("");
		Sleep(2000);
		break;
	case 2:
		...
		drawUI.DrawTextUI(player->getName()+"      ݷ           " + to_string(150) + "%        ߽  ϴ ", "");
		drawUI.DrawInputUI("");
		Sleep(2000);
		if (rand <= 15) {
			player->takeDamage(player->getAttackMin());
			drawUI.DrawTextUI(to_string(15) + "    Ȯ      ڱ   ڽ         ߽  ϴ .", "");
			drawUI.DrawInputUI("");
		}
		drawUI.DrawStatus(player->getName(), "", to_string(player->getHealth()), "");
		Sleep(2000);
	...
	case 3:
		...
		drawUI.DrawTextUI("        ÷  ̾             Բ      ˴ϴ .", "               "+to_string(25)+"% Ȯ      ƹ  ͵                  ְ   ˴ϴ .");
		drawUI.DrawInputUI("");
		Sleep(2000);
		break;
	default:
...
void GameManager::Turn(int *isPlayer,State *player, State *enemy)
{
	...
	if (*isPlayer == 1) {
		...
		if (player->getControlByBoss() == Action) {
			drawUI.DrawTextUI("                   ߽  ϴ .", "");
			drawUI.DrawInputUI("    Ϸ    Enter");
			cin.ignore();
			cin.get();
			cout << "\033[1F";
			(*isPlayer)++;
