...
void GameManager::Turn(int *isPlayer,State *player, State *enemy)
{
	...
	if (*isPlayer == 1) {
		...
		if (enemyChance >= ENEMY_ATTACK_CHANCE) {
			drawUI.DrawTextUI(STRING_TURN(player->getName())+"               "+enemy->getName() + "은(는) 대기하려고 한다.", STRING_SELECT_SKILL_LIST);
		}
		else { 
			drawUI.DrawTextUI(STRING_TURN(player->getName())+ "               " + enemy->getName() + "은(는) 공격하려고 한다.", STRING_SELECT_SKILL_LIST);
		}
